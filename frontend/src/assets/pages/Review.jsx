    ...
    return (
        <header className="sticky top-0 left-0 right-0 z-[990] flex items-center justify-between border-b border-primary/20 bg-background-light/80 backdrop-blur-md px-6 md:px-20 py-4">
            ...
            <div className="flex items-center gap-4">
                <div onClick={() => navigate('/profile')} className="flex items-center gap-2 text-navy-deep/70">
                    <span className="material-symbols-outlined text-navy-deep/40">account_circle</span>
                    <span className="text-sm font-medium">Nguyễn Khách</span>
                </div>
...
const ReviewForm = ({ onReviewSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!email || !content) return alert("Vui lòng điền đầy đủ thông tin!");

        setLoading(true);
        try {
            await axios.post(`${API_URL}/danh-gia`, {
                Email: email,
                NoiDung: content,
                SoSao: rating
            });
    ...
    return (
        <section className="marble-bg gold-border-gradient rounded-xl shadow-2xl p-8 md:p-12 mb-16 relative z-80 -mt-20 bg-white">
            ...
            <form className="grid grid-cols-1 md:grid-cols-2 gap-8" onSubmit={handleSubmit}>
                <div className="flex flex-col gap-6">
                    <label className="group">
                        <span className="block text-xs uppercase tracking-widest font-bold text-navy-deep/70 mb-2">Email Address</span>
                        <input
                            className="w-full bg-transparent border-0 border-b border-navy-deep/20 focus:ring-0 focus:border-primary py-3 transition-all placeholder:text-navy-deep/30 font-display italic outline-none"
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex gap-2 ">
                            {[1, 2, 3, 4, 5].map((star) => (
                                ...
                                >
                                    <span
                                        className={`material-symbols-outlined text-3xl  ${star <= rating ? 'text-primary' : 'text-primary/30'}`}
                ...
                <div className="flex flex-col">
                    <label className="flex-1 flex flex-col h-full">
                        ...
                        <textarea
                            className="w-full flex-1 min-h-[150px] bg-transparent border border-navy-deep/10 rounded-lg focus:ring-1 focus:ring-primary focus:border-primary p-4 transition-all placeholder:text-navy-deep/30 font-display leading-relaxed outline-none resize-none"
                ...
                <div className="md:col-span-2 flex justify-center mt-4">
                    <button
                        type="submit"
                        disabled={loading}
                        className={`${loading ? 'opacity-50' : 'hover:shadow-primary/40 hover:-translate-y-0.5'} bg-primary text-navy-deep px-12 py-4 rounded font-bold uppercase tracking-[0.3em] text-xs shadow-lg shadow-primary/20 transition-all`}
                    >
...
const Testimonials = ({ reviews }) => {
    // Chốt chặn lỗi .map: Đảm bảo safeReviews luôn là mảng
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-navy-deep/5 flex items-center justify-center border border-primary/20">
                                    <span className="material-symbols-outlined text-navy-deep/40">person</span>
                                </div>
                                <div>
                                    <p className="font-display font-bold text-navy-deep">{testimonial.Email}</p>
...
export default function ReviewPage() {
    ...
    const fetchReviews = async () => {
        try {
            const response = await axios.get(`${API_URL}/danh-gia`);
            // Laravel có thể trả về mảng trực tiếp hoặc nằm trong response.data.data
